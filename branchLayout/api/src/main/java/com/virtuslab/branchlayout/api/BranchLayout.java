...
public class BranchLayout implements IBranchLayout {
  ...
  public IBranchLayout slideIn(String parentBranchName, IBranchLayoutEntry entryToSlideIn)
    ...
    if (isDescendant(/* presumedAncestor */ entry, /* presumedDescendant */ parentEntry)) {
  ...
  private static boolean isDescendant(@Nullable IBranchLayoutEntry presumedAncestor, IBranchLayoutEntry presumedDescendant) {
    if (presumedAncestor != null && presumedAncestor.getChildren().contains(presumedDescendant)) {
      return true;
    } else {
      return presumedAncestor != null && presumedAncestor.getChildren().exists(e -> isDescendant(e, presumedDescendant));
    }
  }

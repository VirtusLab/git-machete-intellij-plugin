...
public class StatusTreeItem extends TreeItem<Text> {
  ...
  private void setValue(List<String> items) {
    ...
    List<TreeItem<Text>> statusItems =
        items.stream().map(Text::new).map(TreeItem::new).collect(Collectors.toList());
  ...
  private void lightStyle(Text branchNameText) {
    ...
    branchNameText.setFont(
        Font.font(Font.getDefault().getFamily(), FontWeight.THIN, Font.getDefault().getSize()));
  }

  private void boldStyle(Text lastCommitText) {
    lastCommitText.setFill(Color.BLACK);
    lastCommitText.setFont(
  ...
  public void setListCommits(boolean value) {
    ...
    if (value) {
      ...
      ObservableList<TreeItem<Text>> children =
          this.getChildren().get(this.getChildren().size() - 1).getChildren();
      this.getChildren().clear();

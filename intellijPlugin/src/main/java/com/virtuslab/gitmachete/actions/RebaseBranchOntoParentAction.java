...
public class RebaseBranchOntoParentAction extends AnAction {
  ...
  private Optional<IGitRebaseParameters> computeGitRebaseParameters(
    ...
    if (gitMacheteCurrentBranch == null) return Optional.empty();
   ...
   * {@link com.intellij.openapi.vcs.ProjectLevelVcsManager#checkVcsIsActive(String)} which is true
   * when the specified VCS is used by at least one module in the project. Therefore it is
   * guaranteed that while the Git Machete plugin tab is visible, a git repository exists.
   */

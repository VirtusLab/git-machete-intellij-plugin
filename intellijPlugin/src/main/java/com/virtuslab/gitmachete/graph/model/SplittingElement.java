 ...
 * an attach point for next (sibling) branch (for a graph with commits an attach point would be that
 * last {@link CommitElement}). In particular, it is an intersection for two direct children of the
 * same parent branch (siblings). For a given parent branch all its splitting elements are
 * horizontally aligned (with the parent, and each other). Thus all its child branches will be
 * aligned too, but one unit right from their splitting elements (indented). The only exception from
 * it is the last branch which is not indented because of lack the of next (sibling) branch. The

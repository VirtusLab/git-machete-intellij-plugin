...
public class GitMacheteUI extends JFXPanel {
  ...
  private void populateTree() {
    ...
    StatusTreeItem allow =
        new StatusTreeItem("Allow ownership links", "1st round of fixes", "allow-ownership-link");
    StatusTreeItem build = new StatusTreeItem("Build arbitrarily long chains", "build-chain");
    StatusTreeItem call =
        new StatusTreeItem(
            "Call web service", "1st round of fixes", "2nd round of fixes", "call-ws");
    StatusTreeItem master = new StatusTreeItem("master");
    StatusTreeItem hotfix =
        new StatusTreeItem("HOTFIX Add the trigger (amended)", "hotfix/add-trigger");

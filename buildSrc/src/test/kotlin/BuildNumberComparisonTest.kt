...
class BuildNumberComparisonTest {
    ...
    fun shouldRecognizeEqualVersions() {
        ...
        val version2 = "0.0.0"
        assert(!version1.buildNumberIsNewerThan(version1))
        assert(!version2.buildNumberIsNewerThan(version2))
    }

    @Test
    fun shouldRecognizeNotEqualVersions() {
        val olderVersion = "1111.11.111"
        val newerVersions = listOf("1112.11.111",
            "1111.12.111",
            "1111.11.112",
            "1111.11.111.0")

        for (newerVersion in newerVersions)
            assert(newerVersion.buildNumberIsNewerThan(olderVersion))
    }
}
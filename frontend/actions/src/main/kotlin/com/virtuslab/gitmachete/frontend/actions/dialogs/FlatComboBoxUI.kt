 ...
 */
internal class FlatComboBoxUI(
    var border: Insets = Insets(1, 1, 1, 1),
    var outerInsets: Insets = JBInsets.create(DarculaUIUtil.BW.get(), DarculaUIUtil.BW.get()),
    @NlsContexts.StatusText val popupEmptyText: String = StatusText.getDefaultEmptyText(),
    val popupComponentProvider: ((JComponent) -> JComponent)? = null
  ...
  override fun paintArrow(g2: Graphics2D, btn: JButton) {
    ...
    val leftLine =
        Line2D.Float(
            xU.toFloat(),
            yU.toFloat(),
            xU.toFloat() + tW.toFloat() / 2f,
            yU.toFloat() + tH.toFloat())

    val rightLine =
        Line2D.Float(
            xU.toFloat() + tW.toFloat() / 2f,
            yU.toFloat() + tH.toFloat(),
            xU.toFloat() + tW.toFloat(),
            yU.toFloat())
  ...
  override fun getInnerShape(r: Rectangle, bw: Float, lw: Float, arc: Float): RectangularShape {
    return Rectangle2D.Float(
        outerInsets.left + lw * border.left,
        outerInsets.top + lw,
  ...
  private fun configureList(list: JList<*>) {
    (list as? ComponentWithEmptyText)?.let { it.emptyText.text = popupEmptyText }
  }

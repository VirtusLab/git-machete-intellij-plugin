...
@CustomLog
public abstract class BaseRebaseAction extends BaseGitMacheteRepositoryReadyAction
    implements
      IBranchNameProvider,
  ...
  @UIEffect
  protected abstract void setProperSuccessPresentation(AnActionEvent anActionEvent);
  ...
  protected void onUpdate(AnActionEvent anActionEvent) {
    ...
    if (state == null) {
      ...
      presentation.setDescription(
          getNonHtmlString("action.GitMachete.BaseRebaseAction.description.disabled.repository.unknown-state"));
      ...
      val stateName = Match(state).of(
          Case($(Repository.State.GRAFTING),
              getString("action.GitMachete.BaseRebaseAction.description.repository.state.ongoing.cherry-pick")),
          Case($(Repository.State.DETACHED),
              getString("action.GitMachete.BaseRebaseAction.description.repository.state.detached-head")),
          Case($(Repository.State.MERGING),
              getString("action.GitMachete.BaseRebaseAction.description.repository.state.ongoing.merge")),
          Case($(Repository.State.REBASING),
              getString("action.GitMachete.BaseRebaseAction.description.repository.state.ongoing.rebase")),
          Case($(Repository.State.REVERTING),
              getString("action.GitMachete.BaseRebaseAction.description.repository.state.ongoing.revert")),
          Case($(), ": " + state.name().toLowerCase()));

      presentation.setEnabled(false);
      presentation.setDescription(
          getNonHtmlString("action.GitMachete.BaseRebaseAction.description.disabled.repository.status")
              .format(stateName));
    } else {
      ...
      if (isCalledFromContextMenu && isRebasingCurrent) {
        setProperSuccessPresentation(anActionEvent);
      }
    }
  }

  protected abstract boolean isRebaseTargetPresent(AnActionEvent anActionEvent);
  ...
  public void actionPerformed(AnActionEvent anActionEvent) {
    ...
    if (branch != null) {
      if (isRebaseTargetPresent(anActionEvent)) {
        doRebase(anActionEvent, branch);
      } else {
  ...
  private void doRebase(AnActionEvent anActionEvent, IManagedBranchSnapshot branchToRebase) {
  ...
  protected abstract CheckedFunction0<IGitRebaseParameters> getParametersForRebase(IManagedBranchSnapshot branchToRebase);

  private void doRebase(
      Project project,
      GitRepository gitRepository,
      IGitMacheteRepositorySnapshot gitMacheteRepositorySnapshot,
      IManagedBranchSnapshot branchToRebase,
    ...
    val tryGitRebaseParameters = Try.of(getParametersForRebase(branchToRebase));
    ...
    if (tryGitRebaseParameters.isFailure()) {
      ...
      VcsNotifier.getInstance(project).notifyError(/* displayId */ null,
          getString("action.GitMachete.BaseRebaseAction.notification.title.rebase-fail"), message);
      return;
    }
    ...
    new Task.Backgroundable(project, getString("action.GitMachete.BaseRebaseAction.hook.task-title")) {
      @Override
      @UIThreadUnsafe
      public void run(ProgressIndicator indicator) {
        ...
        if (hookResult.isFailure()) {
          ...
          LOG.error(message);
          VcsNotifier.getInstance(project).notifyError(/* displayId */ null,
              getString("action.GitMachete.BaseRebaseAction.notification.title.rebase-abort"),
              message);
          return;
        }
        ...
        new Task.Backgroundable(project, getString("action.GitMachete.BaseRebaseAction.task-title")) {
          @Override
          @UIThreadUnsafe
          public void run(ProgressIndicator indicator) {

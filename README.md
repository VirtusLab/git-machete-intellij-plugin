...
<details>
<summary>Where to find the plugin tab</summary>
...
It's located inside a `.git` directory in your repo.

This branch layout can be automatically discovered based on your git repository by the `Discover Branch Layout` action.
...
<details>
<summary>Edit machete file</summary>
...
Two of them (`allow-ownership-link` and `build-chain`) have custom annotation - it's some kind of short description for a given branch and it can be any text that will be displayed next to this branch - in this case, they are pull request numbers.
...
If the edge between them is green that means child branch is in sync with its parent branch - in other words, there are no commits in parent branch that don't belong to the child.
But if there are some commits in the parent branch that are **not** reachable from the child, then the edge is red - we need to [rebase](#rebase) child branch onto the parent.
...
<details>
<summary>Check out branches</summary>
...
<details>
<summary>Listing commits</summary>
...
Basically, we need force push in a case when we rebased our branch because local and remote branch diverged from each other (they have different commits), so git won't let us do regular push.
...
<details>
<summary>Reset to remote</summary>

If a given branch is not in sync with its remote, we can reset it to point to the exact same commit as the remote.
...
<details>
<summary>Fast forward parent</summary>
...
Fast forward merge is nothing else like move tip of a branch to which we merge to the same commit as a tip of a merged branch.
This is what `Fast Forward Parent Branch To Match Current Branch` context menu action does.
As you can see, the edge between a given branch and its parent was changed to grey - it means that the child branch has been merged.

![](docs/fast_forward.gif)
</details>

<details>
<summary>Slide out branch</summary>
...
This action will modify a branch layout for you - delete the selected branch and reattach its children as children of deleted branch parent.
When the `machete.slideOut.deleteLocalBranch` git configuration key is set to `true`, then this action delete also sided out a branch from your git repository.
...
<details>
<summary>Slide in branch</summary>

You can also create a new branch bellow selected one (also without manual editing `machete` file).
To do this, choose the `Slide In Branch Bellow Selected Branch` action from a context menu.
...
Again, select the `Slide In Branch Bellow Selected Branch` from a context menu and type already existent branch name.
...
<details>
<summary>Multi-repository support</summary>

Git Machete supports many git repositories in one project. It can be many regular repositories or submodules.
...
<details>
<summary>Generate plugin zip</summary>

...
Version updates can be done automatically using `./gradlew updateIntellijVersions`, this runs two update tasks described in the example below.
...
2. then IntelliJ `2020.3-EAP` is released (see [snapshot repository](https://www.jetbrains.com/intellij-repository/snapshots/) -> Ctrl+F `.idea`),
   and is detected with `./gradlew updateEapBuildNumber`, <br/>
   new `eapOfLatestSupportedMajor` is set in [IntellijVersions](buildSrc/src/main/kotlin/com/virtuslab/gitmachete/buildsrc/IntellijVersions.kt)
...
5. new stable version `2020.3` is released (see [release repository](https://www.jetbrains.com/intellij-repository/releases/) -> Ctrl+F `.idea`)
   and is detected using `./gradlew updateIntellijStableVersions`, <br/>
   `latestStable` and additionally `latestMinorsOfOldSupportedMajors` are changed in [IntellijVersions](buildSrc/src/main/kotlin/com/virtuslab/gitmachete/buildsrc/IntellijVersions.kt)

...
3. we check if `0.7.0` is compatible with IntelliJ `2020.3-EAP`:
   set `eapOfLatestSupportedMajor` in [IntellijVersionHelper](buildSrc/src/main/kotlin/com/virtuslab/gitmachete/buildsrc/IntellijVersionHelper.kt)
...
4. we release the plugin as `0.8.0` (`untilBuild` will extend automatically to `2020.3.*`
   via `latestSupportedMajor` in [IntellijVersionHelper](buildSrc/src/main/kotlin/com/virtuslab/gitmachete/buildsrc/IntellijVersionHelper.kt))
